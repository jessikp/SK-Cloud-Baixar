...
const startMessage = `Hi I Upload Magnet Link To Gdrive And Send GDrive Link`;
...
const helpMessage = `Here Are Some Commands To Get You Started:
...
function bot(torrent, bot) {
  ...
  bot.on("message", async msg => {
    ...
    try {
      ...
      const publicLink = `https://hb4allgdrive.herokuapp.com/drive/file/${fileName}?id=${driveId}`;
      bot.sendMessage(chatId, `${fileName} Uploaded Successful\nDrive link: ${driveLink}\nPublic link: ${publicLink}`);
    } catch (e) {
